...
// Mapping and collection content management
function ManageCollectionActivities() {

    this._customHtmlMarkers = [];
    this._removeMarkers();
    this.collection_id = $('#collectionContent').data('collection');

}

ManageCollectionActivities.prototype._fetchData = function() {
...
(function(){
    ...
    let searchInput = $('.collection-contents-search');
    ...
    let mapActivities = new ManageCollectionActivities();
    mapActivities._fetchData()

    document.getElementById('addCollection').addEventListener("click", function(){
        let activityID = $('.activity-id').text();
        // let collectionID = $('#collectionContent').data('collection');
        fetch('/collection/addactivity', {
            method: 'POST',
            body: JSON.stringify({
                collection: mapActivities.collection_id,
                activity: activityID
            })
        }).then((response) => {
            return response.json()
